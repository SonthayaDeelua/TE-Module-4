...
export default {
  ...
  methods: {
    ...
    createReview() {
      // logic to create review: Post need to have body which after apiURL in line 50
      fetch(
        this.apiURL, 
        {
          method: 'POST',
          ...
          body : JSON.stringify(this.review) //tranform this.review to be string
        }
      )
      .then(
        (response) => {if(response.ok) {this.$emit('showReviews')} }
      )
